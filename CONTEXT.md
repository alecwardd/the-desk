...
| **Base Detector** | Stateful Rust detector (absorption, pinch, leg-to-leg, DOM cluster, …). Math stays in reviewed Rust; schema, provenance, and promotion (`candidate` → `shadow` → `active`, human-gated) are Feature Registry–governed. Discovery rides `search_catalog` / catalog descriptors when `[sil].catalog_discovery` is on. The write verb is `feature_registry`. Compact current-state fields ride `get_state`. There is no `get_capsule` tool. |
| **Leg (leg-to-leg profile)** | Swing-anchored rotational volume/delta profile (`detector.leg_to_leg`). A **leg** here is a Track C rotation between confirmed swing highs/lows — not MFE/MAE outcome legs and not `research::ib_campaign` confluence legs. Compact current-leg fields ride `get_state`; optional `leg_started` / `leg_completed` events are not Capsule-mandatory. No specialty `get_leg_profile` tool. |
...
| **Feature-IR** | Typed intermediate representation for Derived Features. Not Turing-complete; live-shadow and historical path labels share one evaluator over Journal Frames (`clock_ms <= asOf`). Evaluation is capped at 57600 frames (~8 hours of 1 Hz NQ+ES), independent of the 8192 live pending-journal drain. Live persist / `get_state` / playbook evaluation hydrate a rolling in-memory window once and append on persist; pending frames win on identity. Session-percentile `n` and dwell fail closed when that bound would drop in-session frames. Globex (~17h) exceeds the cap, so those families fail closed overnight once the mixed NQ+ES journal is longer than ~8 hours and the left edge still sits in that Globex session. Historical baselines fail closed whenever the window is truncated (60-day lookback cannot fit in 8 hours). Historical / research loads use a newest-first `LIMIT cap+1` sentinel — never an unbounded `list_journal_frames` scan. `query_series` / `query_episodes` evaluate unstamped Derived Features in one pass. Catalog-field rules bindings are live-only (hypothesis backtests reject them). SIL-M5c codegen emits five kernel artifacts from one accepted (`active`) descriptor: runtime field (`get_state`), Journal Frame payload key, query dimension, catalog-field rules binding, agent schema. |
...
| **DOM-family** | Event types Capsules key off: `stop_run`, `iceberg_reload`, `pull_intent`, `book_velocity_regime_shift` (registry may extend). SIL-M5e Base Detectors emit these types on the live/backfill path (market-by-price reload inference, not MBO). Capsules are mandatory for these types, including `*_invalidated`. The MM-flow composite is catalog/`get_state` only — it is not a fifth Capsule-mandatory type. |

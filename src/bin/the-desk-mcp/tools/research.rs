...
impl TheDeskMcp {
    ...
    pub(crate) async fn feature_registry(
        ...
        Ok(text_result(serde_json::json!({
            ...
            "note": "Your playbook / your rules say how to use a registered descriptor. Discovery rides search_catalog when [sil].catalog_discovery is true; this verb does not change Base Detector math and does not place orders. An active Derived Feature emits five kernel artifacts (get_state, journal_frames.payload, query_series / query_episodes, catalog-field rules binding, search_catalog / describe_domain). Candidate and shadow stay in featureHits only."
        })))
    }
}

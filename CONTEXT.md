...
| **Base Detector** | Stateful Rust detector (absorption, pinch, …). Math stays in reviewed Rust; schema, provenance, and promotion (`candidate` → `shadow` → `active`, human-gated) are Feature Registry–governed. Discovery rides `search_catalog` / catalog descriptors. The write verb is `feature_registry`. |

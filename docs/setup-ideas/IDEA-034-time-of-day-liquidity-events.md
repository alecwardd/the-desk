...
| DOM / book liquidity (`.depth` files, compact `dom_feature_snapshots`) | **Recent-only** | `.depth` files durable; leftover SQLite `depth_events` prune window **7 days** (not live-ingested, SIL-M3f) | Depth reconstruction | Recent weeks only | **Parked:** true book-side liquidity variant is a later optional stage; this plan uses trade-side proxy only |

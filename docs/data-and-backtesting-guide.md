...
**The single most important fact:** the `.scid`/`.depth` files are the source of truth; the
SQLite `data.db` is a *derived cache* that can always be rebuilt from them. Recording depends
only on Sierra running. Backtests replay the `.scid` files directly — not the database.
...
| --- | --- | --- | --- |
| `raw_ticks` | Ticks ingested from `.scid` | MCP live tail + `backfill_history` | `warm_retention_days` (30); older archived to `X:\TheDesk\archive\*.csv.zst` |
...
> tables (`session_summaries`, `prior_day_levels`, `setups`, `research_hypotheses`,
> `risk_config`) provide the historical *inputs* (RVOL curves, prior levels) the replay needs.
...
2. **Isolated DB:** build a tiny isolated DB so the run never contends with the live writer or
   ...
   ```
   It copies only the small **reference** tables (`session_summaries`, `prior_day_levels`,
   `risk_config`, `setups`, `research_hypotheses`) — **not** `raw_ticks` (the replay reads `.scid`).
   Point the runner/backtest at `<dest.db>`.
...
Historical tools need data in the DB — run `backfill_history` for the window first, and check
`get_research_summary` for the baseline sample.
...
- **`depth_events` grows fastest.** It's the DOM table; bounded by `depth_retention_days`. The `.depth` files are the re-ingestable source. (This is what ballooned the DB to 629 GB; see [depth-events-disk-growth] memory + the ops runbook.)
- **Files are truth, DB is cache.** If the DB is ever lost/corrupt, rebuild from `.scid`/`.depth` via backfill. Keep the periodic `[backup]` snapshots (on `X:`) as restore points.

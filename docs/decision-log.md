...
3. **Write path:** `MarketRouter::persist_journal` writes SQLite first (INSERT OR IGNORE), then the attached `ColdFrameStore`. Duplicate `(frame_second, root)` keeps the first write. Partitions never mix RTH with Globex or NQ with ES. Empty `trading_day` is derived from `clock_ms`. A cold-write failure restores pending **frames** (events already in SQLite) and returns an error so the next persist retries the dump.

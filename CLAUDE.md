...
LAYER 3: MCP Server + LLM Orchestration
 - 122 MCP tools expose pipeline state, rules evaluation, setup lifecycle state, research queries, and data queries
...
| MCP server | `rmcp` crate | 122 MCP tools via stdio transport |
...
│   ├── bin/the-desk-mcp.rs           # MCP server binary (122 tools)

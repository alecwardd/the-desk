...
5. Feature Registry: lifecycle writes use `feature_registry` (`register` at candidate for `kind=baseDetector` or `kind=derivedFeature` with a Feature-IR `program`, `promote` candidate→shadow→active with `traderConfirmation`; optional `unit` / `sessionScope` / `freshness` / `costHint` on register). Discovery via `search_catalog` (`featureHits`) requires `[sil].catalog_discovery = true`. The write verb stays on the default surface and reports `discoveryEnabled`. Do not add a specialty getter. Derived Features use only the five funded Operator Families; unfunded families (including surface lookup) are rejected at declaration time. An accepted (`active`) Derived Feature is codegen'd onto the existing kernel (`get_state`, `query_series` / `query_episodes`, catalog-field rules binding, `search_catalog` / `describe_domain`) — no new specialty MCP tool. Candidate and shadow stay in `featureHits` only. Catalog-field rules bindings are **live-only**; `register_hypothesis` / `run_backtest` reject `CatalogField` because the backfill path does not evaluate Feature-IR.
...
> The Feature Registry write verb is `feature_registry` (research domain; Base Detectors
> and Derived Features / Feature-IR); discovery

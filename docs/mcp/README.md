...
- `get_events` returns formalized rows: lifecycle (`open` → `updated` → `resolved`|`expired`), severity, occurrence + dedup identity, `frame_ref` joining each event to the producing Journal Frame, and `capsuleRef` on DOM-family rows. Trust Level L0.

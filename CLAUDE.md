...
LAYER 3: MCP Server + LLM Orchestration
 - 100 MCP tools expose pipeline state, rules evaluation, setup lifecycle state, research queries, and data queries
...
| MCP server | `rmcp` crate | 100 MCP tools via stdio transport |
...
│   ├── bin/the-desk-mcp.rs           # MCP server binary (100 tools)

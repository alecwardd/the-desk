...
| **backtest-analyst** | Historical only | `backfill_history`, `run_backtest`, `get_backfill_status`, `get_backtest_results`, `compare_backtests`, `compare_sessions`, `get_session_history`, `get_research_summary`, all `query_*` research tools, SIL `query_series` / `query_episodes` / `query_raw` / `run_job` when the catalog_discovery flag is on, `register_hypothesis`, `list_hypotheses`, `summarize_hypothesis_run`, `propose_draft_setup` |

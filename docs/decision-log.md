...
4. **Discovery rides the existing kernel:** `search_catalog` returns `featureHits`; `describe_environment` / `describe_domain` expose registry metadata. No specialty getter, no tenth kernel operator. Trust Ceiling stays **L3**. `feature_registry` has mutation authority for registry rows only — no order authority. Read/query kernel operators remain Trust Level **L0** and cannot mutate the registry.
5. **No catalog/registry entry → no new tool** is enforceable for new detector concepts: specialty detector tools (`get_absorption_events`, `get_pinch_events`, `get_rebid_reoffer_zones`, `get_trade_size_profile`) must cite an active registry id; an unregistered concept such as `detector.iceberg_reload` cannot add a specialty market tool.

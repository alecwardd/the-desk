...
> **Planned restructure:** this monolith will be split into `docs/setup-ideas/` (one file
> per idea + an index + `_template.md`) after the currently queued refactors land. Until
> then, new ideas go here using the template below. When the split happens, hypothesis
> `docReference` anchors and the cross-references in the data/backtesting guide, the
> runbook, and ADR-020 must be repointed in the same change.

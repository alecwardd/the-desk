...
| Setup ideas | `docs/setup-ideas/index.md` | matching `docs/setup-ideas/IDEA-NNN-*.md` and `src/rules/setup_templates.rs` |
...
- Any relevant idea status in `docs/setup-ideas/index.md`? Call `list_hypotheses` and skim
  the verdicts — **never re-test a rejected idea** in its rejected form (IDEA-000 and
  IDEA-012 were rejected as specified but reconstructed in IDEA-020; know which form is
  settled).
...
- Engineering conventions: `CLAUDE.md` (Never-Do list, terminology), `AGENT.md`
  (Common Mistakes, coaching vs advisory phrasing).
- If an unwritten-preference question would change the design, **ask one question at a
  time, highest architectural impact first**. Do not batch ten questions or guess on the
  architecture-changing one.
...
- [ ] **Single-source docs.** No new duplicated rule surfaces — pointers only.

## Output

End the pass with a short note in the conversation:

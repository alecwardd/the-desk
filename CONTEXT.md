...
| **Feature-IR** | Typed intermediate representation for Derived Features. Not Turing-complete; live-shadow and historical path labels share one evaluator over supplied Journal Frames. M5b is declaration-and-test-only (no runtime MCP/router caller). Codegen emitters are a later milestone. |

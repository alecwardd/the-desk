...
**Live zone verification (always surface — do not omit):** On every flow read, explicitly report the current rebid (Buy) and reoffer (Sell) zones from `get_rebid_reoffer_zones` with their **exact price band (low–high), type, and lifecycle status (Fresh / Retested / Held / Failed)** — even when the trader asked about something else. The trader is actively comparing the code's computed zones against what they see on their own chart in real time, so precise band edges matter more than a summary count. If no zones are currently detected, say so explicitly ("no active rebid/reoffer zones this session") rather than dropping the line. This is a standing live-validation task: the goal is to confirm the footprint-detected zones match the trader's eye. (Detection logic and doctrine: `src/pipelines/rebid_reoffer.rs`; reference `docs/setup-ideas/IDEA-020-footprint-rebid-reoffer-lifecycle.md`.)
